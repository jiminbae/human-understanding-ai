# v36: v30 기반 4가지 고도화
#   [1] USE_CALIBRATION 기본값 ON (Platt) — Log Loss 대회에서 캘리브레이션은 필수
#   [2] ExtraTrees를 L0 4번째 모델로 추가 — LGB/XGB/CAT와 낮은 상관, 다양성↑
#       (ExtraTrees는 랜덤성이 높아 fixed params 사용, Optuna 불필요)
#   [3] 롤링 윈도우 14일 → 21/30일 추가 — TE 60일에 맞게 중장기 패턴 포착
#   [4] HRV pNN50 추가 — RMSSD와 함께 자율신경계 상태 보완 측정
#       (수면질·스트레스 예측에 직접적으로 유효한 피처)
import warnings
...
USE_CALIBRATION        = os.environ.get('V12_CALIBRATION',     '1') == '1'  # v36: 기본 ON
...
EXP_TAG      = os.environ.get('V30_EXP_TAG', 'v36_calibration_et_rolling')
    ...
    for ti, target in enumerate(TARGETS):
        ...
        for seed in seeds:
            ...
            for tr_idx, val_idx in skf.split(X_train_use, y):
                ...
                # --- [v30] fold 내부 cross-target 피처 주입 ---
                ...
                # v36: ExtraTrees — 랜덤성이 높아 fixed params 사용 (Optuna 불필요)
                ...
                m_et.fit(X_tr.fillna(X_tr.median()), y_tr)   # v36: ET는 NaN 불가, median impute
                ...
                s_oof_et[val_idx]   = m_et.predict_proba(X_val.fillna(X_val.median()))[:, 1]  # v36
                ...
                s_test_et  += m_et.predict_proba(X_te.fillna(X_te.median()))[:, 1] / n_folds  # v36
