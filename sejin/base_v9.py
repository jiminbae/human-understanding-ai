...
# v9 Stable Master: 12-Seed Ensemble + Deep Optuna (30) + Safe Stacking (No Rank Average)
...
def train_and_predict(train_full, test_full, feature_cols):
    ...
    # 🚀 수정 1: 과적합 방지를 위해 Optuna 탐색 횟수를 30회로 안정화
    n_optuna_trials = 30 
    seeds = [42, 1234, 9999, 7, 314, 2025, 777, 555, 1111, 2222, 3333, 4444] 
    n_folds = 5
    te_windows = [3, 7, 14, 21]

    # 메타 모델 파라미터 (단순하고 안전한 Logistic Regression 유지)
    ...
    for ti, target in enumerate(TARGETS):
        ...
        print(f"  [Optuna] Searching for Golden Parameters (Trials: {n_optuna_trials})...")
        ...
        # 🚀 수정 2: Rank Average 제거 및 순수 확률 산술 평균 복구 (Log-Loss 평가 최적화)
        target_test_lgb = np.mean(seed_test_lgb_list, axis=0)
        target_test_xgb = np.mean(seed_test_xgb_list, axis=0)
        target_test_cat = np.mean(seed_test_cat_list, axis=0)
...
def main():
    ensure_dirs()
    print('Starting Hybrid Pipeline (v9 Stable Master)...')
    ...
    print(f'\n{"=" * 55}\nMaster v9 Stable Total OOF: {oof_total:.4f}\n{"=" * 55}')
