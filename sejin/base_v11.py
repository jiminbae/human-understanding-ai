# v30: v29 + cross-target OOF chaining (2-pass training)
#   - v29 bidirectional target history encoding을 유지
#   - [신규] 2-pass 학습: Pass1에서 7개 타겟 독립 학습 → Pass2에서 나머지 6개 타겟의
#     OOF 예측값을 추가 피처로 사용해 재학습
#   - Q1/Q2/Q3/S1~S4는 강하게 상관(피로↔수면질, 수면지연↔스트레스 등)되어 있으므로
#     cross-target 정보가 각 타겟 예측 성능을 유의미하게 향상시킴
#   - fold 내부에서 tr_idx/val_idx에 맞게 cross OOF를 슬라이싱해 leakage 방지
#   - test 예측에는 Pass1의 test 예측(전체 train 기반)을 cross 피처로 사용
#   - V30_CROSS_TARGET 환경변수로 on/off 제어 (기본 on)
import warnings
...
EXP_TAG      = os.environ.get('V30_EXP_TAG', 'v30_cross_target_chaining')
