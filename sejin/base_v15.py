# v37: v36 버그 수정 + GroupKFold 도입
#   [FIX1] Calibration 기본 OFF — OOF 편향 시 Platt이 오히려 악화시킴
#   [FIX2] ET NaN imputation을 train median으로 통일 (val/test에도 동일 적용)
#   [NEW]  StratifiedKFold → GroupKFold (group=subject_id)
#           같은 subject 데이터가 train/val에 섞이면 OOF가 낙관적으로 나와
#           실제 리더보드와 갭 발생. subject 단위 분리로 진짜 일반화 성능 측정
import warnings
...
USE_CALIBRATION        = os.environ.get('V12_CALIBRATION',     '0') == '1'  # v37: 기본 OFF (OOF 편향 시 악화)
...
USE_CROSS_TARGET    = os.environ.get('V30_CROSS_TARGET', '1') == '1'
...
EXP_TAG      = os.environ.get('V30_EXP_TAG', 'v37_groupkfold_fix')
...
def extract_hr(df_raw, keys):
    ...
    for (sid, d), grp in df_day.groupby(['subject_id', 'date']):
        ...
        # v36: pNN50 — 50ms 초과 연속 심박 간격 차이 비율 (부교감신경 활성도)
        if len(hr) > 5:
            diffs = np.abs(np.diff(hr))
            row['hr_day_pnn50'] = float((diffs > 50).mean())
        else:
            row['hr_day_pnn50'] = np.nan
...
def build_feature_table(train_df, sub_df):
    ...
    for col in roll_cols:
        ...
        # v36: 중장기 롤링 — TE 60일 윈도우와 맞춤
        feat_all[f'{col}_roll21']= g.transform(lambda x: x.shift(1).rolling(21, min_periods=3).mean())
        feat_all[f'{col}_roll30']= g.transform(lambda x: x.shift(1).rolling(30, min_periods=5).mean())
...
def _train_pass(train_full, test_full, feature_cols,
    ...
    for ti, target in enumerate(TARGETS):
        ...
        # --- [v30] Cross-target OOF 피처 준비 (Pass2에서만 사용) ---
        ...
        if has_cross:
            ...
            print(f'  [v30] Cross-target features added: {cross_cols}')
        else:
        ...
        # --- L0: GroupKFold ensemble (subject 단위 분리) ---
        # v37: StratifiedKFold → GroupKFold(subject_id)
        # 같은 subject 데이터가 train/val에 섞이면 OOF가 낙관적 → 리더보드 갭 발생
        print(f'  [L0] Training {len(seeds)}-seed ensemble (GroupKFold by subject)...')
        groups = train_full['subject_id'].values  # fold 분리 기준
        oof_lgb  = np.zeros(len(X_train_base))
        oof_xgb  = np.zeros(len(X_train_base))
        oof_cat  = np.zeros(len(X_train_base))
        oof_et   = np.zeros(len(X_train_base))
        test_lgb = np.zeros(len(X_test_base))
        test_xgb = np.zeros(len(X_test_base))
        test_cat = np.zeros(len(X_test_base))
        test_et  = np.zeros(len(X_test_base))

        for seed in seeds:
            # GroupKFold은 shuffle이 없으므로 seed마다 groups를 shuffle해서 다양성 확보
            unique_subjects = np.unique(groups)
            rng = np.random.RandomState(seed)
            shuffled_subjects = rng.permutation(unique_subjects)
            # subject → fold 매핑
            subj_fold = {s: i % n_folds for i, s in enumerate(shuffled_subjects)}
            fold_indices = np.array([subj_fold[g] for g in groups])

            s_oof_lgb  = np.zeros(len(X_train_base))
            s_oof_xgb  = np.zeros(len(X_train_base))
            s_oof_cat  = np.zeros(len(X_train_base))
            s_oof_et   = np.zeros(len(X_train_base))
            s_test_lgb = np.zeros(len(X_test_base))
            s_test_xgb = np.zeros(len(X_test_base))
            s_test_cat = np.zeros(len(X_test_base))
            s_test_et  = np.zeros(len(X_test_base))

            for fold in range(n_folds):
                tr_idx  = np.where(fold_indices != fold)[0]
                val_idx = np.where(fold_indices == fold)[0]
                ...
                # v37: ET median을 train으로 고정 (val/test에도 동일 적용 → 일관성)
                ...
                m_et.fit(X_tr.fillna(tr_median), y_tr)  # v37: train median으로 impute
                ...
                s_oof_et[val_idx]   = m_et.predict_proba(X_val.fillna(tr_median))[:, 1]  # v37
                ...
                s_test_et  += m_et.predict_proba(X_te.fillna(tr_median))[:, 1] / n_folds  # v37

            oof_lgb  += s_oof_lgb  / len(seeds)
            oof_xgb  += s_oof_xgb  / len(seeds)
            oof_cat  += s_oof_cat  / len(seeds)
            oof_et   += s_oof_et   / len(seeds)   # v36
            test_lgb += s_test_lgb / len(seeds)
            test_xgb += s_test_xgb / len(seeds)
            test_cat += s_test_cat / len(seeds)
            test_et  += s_test_et  / len(seeds)   # v36

        print(f'  [L0] LGBM={log_loss(y,oof_lgb):.4f}  XGB={log_loss(y,oof_xgb):.4f}  CAT={log_loss(y,oof_cat):.4f}  ET={log_loss(y,oof_et):.4f}')

        # --- L1: meta stacking with StratifiedKFold ---
        print('  [L1] Meta stacking...')
        X_meta_tr = np.column_stack([oof_lgb,  oof_xgb,  oof_cat,  oof_et])   # v36: 4모델
        X_meta_te = np.column_stack([test_lgb, test_xgb, test_cat, test_et])   # v36: 4모델
...
def train_and_predict(train_full, test_full, feature_cols):
    ...
    if not USE_CROSS_TARGET:
        print('[v30] USE_CROSS_TARGET=0 → Pass2 생략, Pass1 결과 반환')
        return oof_pass1, test_pass1
