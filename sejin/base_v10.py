# v24: v23 + 속도 및 파이프라인 최적화
#   - 시드 앙상블 대폭 축소 (12개 -> 3개)
#   - Optuna Fold-level Pruning 도입 (안 좋은 파라미터 조기 종료)
#   - Optuna SQLite 스토리지 캐싱 도입 (이전 탐색 결과 재사용)
import warnings
...
# [최적화 3] Optuna 캐싱을 위한 SQLite Database 경로 설정
OPTUNA_DB_PATH = OUTPUTS_DIR / 'optuna_studies.db'
OPTUNA_DB_URL  = f"sqlite:///{OPTUNA_DB_PATH}"

EXP_TAG      = 'v24_pipeline_optimized'
...
def train_and_predict(train_full, test_full, feature_cols):
    ...
    # Optuna 공통 Pruner (Median Pruner)
    ...
    for ti, target in enumerate(TARGETS):
        ...
        def objective_lgb(trial):
            ...
            # [최적화 2] Fold-Level Pruning 적용
            for fold, (tr_idx, val_idx) in enumerate(tune_skf.split(X_train_use, y)):
                ...
                loss = log_loss(y[val_idx], m.predict_proba(X_train_use.iloc[val_idx])[:,1])
                losses.append(loss)
                
                # 조기 종료 체크
                trial.report(np.mean(losses), fold)
        ...
        # [최적화 3] DB 스토리지와 연동하여 Optuna 캐싱
        study_lgb = optuna.create_study(direction='minimize', study_name=f'{EXP_TAG}_lgb_{target}', storage=OPTUNA_DB_URL, load_if_exists=True, pruner=pruner)
        study_xgb = optuna.create_study(direction='minimize', study_name=f'{EXP_TAG}_xgb_{target}', storage=OPTUNA_DB_URL, load_if_exists=True, pruner=pruner)
        study_cat = optuna.create_study(direction='minimize', study_name=f'{EXP_TAG}_cat_{target}', storage=OPTUNA_DB_URL, load_if_exists=True, pruner=pruner)

        study_lgb.optimize(objective_lgb, n_trials=n_optuna_trials)
