# v26: v23 + Temporal Block CV + Subject-level Calibration
#   - Temporal Block CV: StratifiedKFold 대신 subject별 시간순 5분위 fold
#     (test가 훈련 날짜 사이에 block으로 끼어있는 구조를 반영)
#   - Subject Calibration: 전체 모델 학습 후 subject별 LogisticRegression으로 개인 보정
#     (OOF 예측값으로 fit → test 예측값에 적용)
import warnings
...
SUBJ_CAL_MIN_SAMPLES = int(os.environ.get('V26_CAL_MIN_N', '15'))
...
EXP_TAG      = 'v26_temporal_cv_subj_cal'
...
def build_temporal_cv_folds(train_full, n_folds=5):
    """
    test 날짜가 훈련 날짜 사이에 block으로 끼어있는 구조를 반영한 CV.
    각 subject의 훈련 데이터를 시간순으로 n_folds 등분해 fold 배정.
    fold 0 = 가장 최근 구간 (validation 우선), fold n_folds-1 = 가장 오래된 구간.
    반환: positional index array (train_full의 iloc 인덱스로 사용).
    """
    n = len(train_full)
    fold_assignment = np.zeros(n, dtype=int)
    pos_arr = np.arange(n)

    for subj in train_full['subject_id'].unique():
        mask     = (train_full['subject_id'] == subj).values
        subj_pos = pos_arr[mask]
        dates    = train_full['lifelog_date'].values[mask]
        order    = np.argsort(dates)
        sorted_pos = subj_pos[order]

        n_subj   = len(sorted_pos)
        segment  = np.floor(np.arange(n_subj) / n_subj * n_folds).astype(int)
        segment  = np.clip(segment, 0, n_folds - 1)
        fold_vals = n_folds - 1 - segment   # 오래된 구간 → 높은 fold 번호, 최신 → fold 0
        fold_assignment[sorted_pos] = fold_vals
...
def train_and_predict(train_full, test_full, feature_cols):
    ...
    # --- Temporal block CV: subject별 시간순 분할 (한 번만 계산) ---
    tune_fold_assign = build_temporal_cv_folds(train_full, n_folds=3)
    main_fold_assign = build_temporal_cv_folds(train_full, n_folds=n_folds)
    tune_folds = [(np.where(tune_fold_assign != k)[0], np.where(tune_fold_assign == k)[0])
                  for k in range(3)]
    main_folds = [(np.where(main_fold_assign != k)[0], np.where(main_fold_assign == k)[0])
    ...
    for ti, target in enumerate(TARGETS):
        ...
        # --- Optuna tuning (3-fold temporal CV) ---
        ...
        # --- L0: 12-seed temporal CV ensemble ---
        ...
        for seed in seeds:
            ...
            for tr_idx, val_idx in main_folds:
        ...
        # --- L1: meta stacking with temporal CV ---
        ...
        for tr_idx, val_idx in main_folds:
            ...
            mm.fit(X_meta_tr[tr_idx], y[tr_idx])
            meta_oof[val_idx]  = mm.predict_proba(X_meta_tr[val_idx])[:, 1]
...
def apply_subject_calibration(oof_preds, test_preds, train_full, test_full):
    """B: subject별 Platt scaling — OOF 예측으로 fit, test에 적용."""
    ...
    for ti, target in enumerate(TARGETS):
        y = train_full[target].values
        for subj in train_full['subject_id'].unique():
            tr_mask = (train_full['subject_id'] == subj).values
            te_mask = (test_full['subject_id']  == subj).values

            n_tr = tr_mask.sum()
            if n_tr < SUBJ_CAL_MIN_SAMPLES or te_mask.sum() == 0:
                continue
            y_subj = y[tr_mask]
            if len(np.unique(y_subj)) < 2:
                continue

            X_cal = oof_preds[tr_mask, ti].reshape(-1, 1)
            cal   = LogisticRegression(C=0.5, solver='lbfgs', max_iter=1000)
            cal.fit(X_cal, y_subj)

            oof_cal[tr_mask, ti]  = cal.predict_proba(X_cal)[:, 1]
            test_cal[te_mask, ti] = cal.predict_proba(
                test_preds[te_mask, ti].reshape(-1, 1))[:, 1]
            n_calibrated += 1

    print(f'  [SubjCal] {n_calibrated} (subject × target) pairs calibrated '
          f'(min_samples={SUBJ_CAL_MIN_SAMPLES})')
...
def main():
    ...
    print(f'\n{"="*55}')
    print(f'{EXP_TAG} [Global] Total OOF:          {oof_total:.4f}')
    print(f'{EXP_TAG} [Global] Pseudo-public OOF:  {pseudo_oof_total:.4f}')
    print(f'{"="*55}')
    for t in TARGETS:
        print(f'  {t}: OOF={per_target[t]:.4f}  pseudo={pseudo_per_target[t]:.4f}')

    # --- B: Subject-level calibration ---
    print(f'\n[SubjCal] Applying subject-level calibration...')
    oof_cal, test_cal = apply_subject_calibration(oof_preds, test_preds, train_full, test_full)

    per_target_cal = {t: log_loss(train_full[t].values, oof_cal[:, i])
                      for i, t in enumerate(TARGETS)}
    oof_total_cal  = float(np.mean(list(per_target_cal.values())))
    pseudo_per_target_cal = {t: log_loss(train_full.loc[pseudo_mask, t].values, oof_cal[pseudo_mask, i])
                             for i, t in enumerate(TARGETS)}
    pseudo_oof_total_cal  = float(np.mean(list(pseudo_per_target_cal.values())))

    print(f'\n{"="*55}')
    print(f'{EXP_TAG} [Calibrated] Total OOF:          {oof_total_cal:.4f}')
    print(f'{EXP_TAG} [Calibrated] Pseudo-public OOF:  {pseudo_oof_total_cal:.4f}')
    print(f'{"="*55}')
    for t in TARGETS:
        print(f'  {t}: OOF={per_target_cal[t]:.4f}  pseudo={pseudo_per_target_cal[t]:.4f}')
    ...
    for i, t in enumerate(TARGETS):
        submission[t] = test_cal[:, i].clip(0.02, 0.98)
