# 정세진의 base_v4 기반 수정
# 변경사항:
#   1. USE_TRAIN_SUBJ_NORM: 피험자별 z-score를 train fold 통계 기반으로 계산 (base_v4 구현 유지)
#   2. USE_RANK_BLEND: 확률 예측과 rank 정규화 예측을 7:3으로 블렌딩 (v10에서 효과 확인)
...
USE_TRAIN_SUBJ_NORM = os.environ.get('V14_TRAIN_NORM', '0') == '1'
USE_RANK_BLEND      = os.environ.get('V14_RANK_BLEND', '0') == '1'
USE_FOLD_SAFE_TE    = os.environ.get('V14_FOLD_SAFE_TE', '1') == '1'
USE_CALIBRATION     = os.environ.get('V14_CALIBRATION', '0') == '1'
CALIBRATION_METHOD  = os.environ.get('V14_CALIB_METHOD', 'platt').strip().lower()
if CALIBRATION_METHOD not in {'platt', 'isotonic'}:
    raise ValueError("V14_CALIB_METHOD must be one of: platt, isotonic")
PSEUDO_PUBLIC_TAIL_FRAC = float(os.environ.get('V14_PSEUDO_TAIL_FRAC', '0.2'))
if not (0 < PSEUDO_PUBLIC_TAIL_FRAC < 1):
    raise ValueError("V14_PSEUDO_TAIL_FRAC must be in (0, 1)")

FORCE_CPU = os.environ.get('V14_FORCE_CPU', '0') == '1'
...
OUTPUT_PATH     = OUTPUT_DIR  / 'submission_v14.csv'
REPORT_PATH     = REPORT_DIR  / 'report_v14.txt'
SUMMARY_PATH    = SUMMARY_DIR / 'summary_v14.json'
OOF_PATH        = OOF_DIR     / 'oof_v14.csv'
TEST_PREDS_PATH = REPORT_DIR  / 'test_preds_v14.csv'
RUN_LOG_PATH    = LOG_DIR     / 'run_v14.log'
...
def write_report(report_data):
    lines = [
        '=' * 80,
        'Baseline v14 run report',
        f"Generated: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        'Base: base_v4 (triple ensemble) + trainnorm + rankblend',
...
def main():
    ...
    print('Starting v14 Training Pipeline...')
    ...
    print(f'\n{"=" * 55}')
    print(f'v14 Total OOF:         {oof_total:.4f}')
    print(f'v14 Pseudo-public OOF: {pseudo_oof_total:.4f}')
    print(f'{"=" * 55}')
    ...
    summary = {
        'exp_tag': 'v14_triple_ensemble_trainnorm_rankblend',
