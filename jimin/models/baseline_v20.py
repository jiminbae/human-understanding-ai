# v20: v19 (Optuna + StratifiedKFold + L1 Stacking) + Daytime HR Features
#   - v19 베이스 (Optuna 50 trials/모델/타겟, StratifiedKFold, L1 meta stacking)
#   - extract_hr 수정: 낮 시간대(6시~자정) 심박수 피처 추가
#     (hr_day_mean/std/rmssd, 시간대별 평균, HR 존 비율, 서카디안 리듬 지표)
import warnings
...
def extract_ble(df_raw, keys):
    ...
    for (sid, d), grp in df_raw.groupby(['subject_id', 'date']):
        ...
        for v in grp['m_ble']:
            if isinstance(v, list):
                for dev in v:
                    if isinstance(dev, dict): addrs.add(dev.get('address', ''))
...
def train_and_predict(train_full, test_full, feature_cols):
    ...
    for ti, target in enumerate(TARGETS):
        ...
        # --- L0: 8-seed StratifiedKFold ensemble ---
        print('  [L0] Training 8-seed ensemble...')
