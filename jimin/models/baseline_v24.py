# v24: v23 + Cross-target stacking (2-phase)
#   - Phase 1: stability filter + 100-trial Optuna per target → phase1 OOF/test + stable_cols_map
#   - Phase 2: base features + 6 cross-target OOF features → 30-trial Optuna → final predictions
#   - Cross-target features: unbiased phase1 OOF (no leakage)
#   - MAX_STABLE_FEATURES: 150 (default)
import warnings
...
EXP_TAG      = 'v24_cross_target_stack'
...
def _run_l0_l1(X_tr_use, X_te_use, train_full, test_full, y, target,
               best_lgb, best_xgb, best_cat, seeds, n_folds,
               meta_params, te_windows, USE_FOLD_SAFE_TE, tag):
    N = len(X_tr_use); M = len(X_te_use)
    oof_lgb  = np.zeros(N); oof_xgb  = np.zeros(N); oof_cat  = np.zeros(N)
    test_lgb = np.zeros(M); test_xgb = np.zeros(M); test_cat = np.zeros(M)

    for seed in seeds:
        skf = StratifiedKFold(n_splits=n_folds, shuffle=True, random_state=seed)
        s_oof_lgb  = np.zeros(N); s_oof_xgb  = np.zeros(N); s_oof_cat  = np.zeros(N)
        s_test_lgb = np.zeros(M); s_test_xgb = np.zeros(M); s_test_cat = np.zeros(M)

        for tr_idx, val_idx in skf.split(X_tr_use, y):
            X_tr  = X_tr_use.iloc[tr_idx].copy()
            X_val = X_tr_use.iloc[val_idx].copy()
            X_te  = X_te_use.copy()
        ...
        oof_lgb  += s_oof_lgb  / len(seeds); oof_xgb  += s_oof_xgb  / len(seeds); oof_cat  += s_oof_cat  / len(seeds)
        test_lgb += s_test_lgb / len(seeds); test_xgb += s_test_xgb / len(seeds); test_cat += s_test_cat / len(seeds)

    print(f'  [{tag} L0] LGBM={log_loss(y,oof_lgb):.4f}  XGB={log_loss(y,oof_xgb):.4f}  CAT={log_loss(y,oof_cat):.4f}')

    print(f'  [{tag} L1] Meta stacking...')
    X_meta_tr = np.column_stack([oof_lgb,  oof_xgb,  oof_cat])
    X_meta_te = np.column_stack([test_lgb, test_xgb, test_cat])
    meta_oof  = np.zeros(N); meta_test = np.zeros(M)
    ...
    print(f'  [{tag} L1] Stacked OOF [{target}]: {log_loss(y, meta_oof):.4f}')
    return meta_oof, meta_test


def _build_best_params(study_lgb, study_xgb, study_cat):
...
def train_and_predict(train_full, test_full, feature_cols):
    ...
    n_folds            = 5
    n_optuna_trials_p1 = int(os.environ.get('V24_OPTUNA_P1', '100'))
    n_optuna_trials_p2 = int(os.environ.get('V24_OPTUNA_P2', '30'))
    te_windows         = [3, 7, 14, 21]

    N = len(X_train_base); M = len(X_test_base)
    phase1_oof      = np.zeros((N, len(TARGETS)))
    phase1_test     = np.zeros((M, len(TARGETS)))
    stable_cols_map = {}

    # ===== Phase 1: per-target stability filter + 100-trial Optuna =====
    print(f'\n{"="*55}\n=== Phase 1: Base Training ({n_optuna_trials_p1} Optuna trials) ===\n{"="*55}')

    for ti, target in enumerate(TARGETS):
        y = train_full[target].values
        print(f'\n{"="*40}\n=== [P1] Target: {target} | pos_rate: {y.mean():.3f} ===\n{"="*40}')

        print('  [Stability] Computing feature stability scores...')
        ...
        stable_cols_map[target] = stable_cols

        print(f'  [Optuna P1] Searching parameters ({n_optuna_trials_p1} trials/model)...')
        tune_skf = StratifiedKFold(n_splits=3, shuffle=True, random_state=42)

        def objective_lgb(trial, X=X_train_use, yy=y):
            params = {
            ...
            losses = []
            for tr_idx, val_idx in tune_skf.split(X, yy):
                m = lgb.LGBMClassifier(**params)
                try: m.fit(X.iloc[tr_idx], yy[tr_idx])
                except:
                    params['device'] = 'cpu'; m = lgb.LGBMClassifier(**params)
                    m.fit(X.iloc[tr_idx], yy[tr_idx])
                losses.append(log_loss(yy[val_idx], m.predict_proba(X.iloc[val_idx])[:,1]))
            return np.mean(losses)

        def objective_xgb(trial, X=X_train_use, yy=y):
            params = {
            ...
            for tr_idx, val_idx in tune_skf.split(X, yy):
                ...
                m.fit(X.iloc[tr_idx], yy[tr_idx], verbose=False)
                losses.append(log_loss(yy[val_idx], m.predict_proba(X.iloc[val_idx])[:,1]))
            return np.mean(losses)

        def objective_cat(trial, X=X_train_use, yy=y):
            params = {
        ...
        study_lgb = optuna.create_study(direction='minimize'); study_lgb.optimize(objective_lgb, n_trials=n_optuna_trials_p1)
        study_xgb = optuna.create_study(direction='minimize'); study_xgb.optimize(objective_xgb, n_trials=n_optuna_trials_p1)
        study_cat = optuna.create_study(direction='minimize'); study_cat.optimize(objective_cat, n_trials=n_optuna_trials_p1)
        print(f'    LGBM best: {study_lgb.best_value:.4f}  XGB: {study_xgb.best_value:.4f}  CAT: {study_cat.best_value:.4f}')

        best_lgb, best_xgb, best_cat = _build_best_params(study_lgb, study_xgb, study_cat)

        print(f'  [P1] Training {len(seeds)}-seed ensemble...')
        p1_oof, p1_test = _run_l0_l1(
            X_train_use, X_test_use, train_full, test_full, y, target,
            best_lgb, best_xgb, best_cat, seeds, n_folds,
            meta_params, te_windows, USE_FOLD_SAFE_TE, 'P1')
        phase1_oof[:, ti]  = p1_oof
        phase1_test[:, ti] = p1_test

    # ===== Phase 2: cross-target stacking + 30-trial Optuna =====
    print(f'\n{"="*55}\n=== Phase 2: Cross-Target Stacking ({n_optuna_trials_p2} Optuna trials) ===\n{"="*55}')

    oof_preds  = np.zeros((N, len(TARGETS)))
    test_preds = np.zeros((M, len(TARGETS)))

    for ti, target in enumerate(TARGETS):
        y = train_full[target].values
        print(f'\n{"="*40}\n=== [P2] Target: {target} | pos_rate: {y.mean():.3f} ===\n{"="*40}')

        stable_cols = stable_cols_map[target]
        X_train_use = X_train_base[stable_cols]
        X_test_use  = X_test_base[stable_cols]

        other_ti    = [j for j in range(len(TARGETS)) if j != ti]
        other_names = [f'ct_{TARGETS[j]}' for j in other_ti]
        cross_tr = pd.DataFrame(phase1_oof[:, other_ti],  columns=other_names).reset_index(drop=True)
        cross_te = pd.DataFrame(phase1_test[:, other_ti], columns=other_names).reset_index(drop=True)
        X_train_p2 = pd.concat([X_train_use.reset_index(drop=True), cross_tr], axis=1)
        X_test_p2  = pd.concat([X_test_use.reset_index(drop=True),  cross_te], axis=1)
        print(f'  Features: {len(stable_cols)} base + {len(other_names)} cross-target = {X_train_p2.shape[1]}')

        print(f'  [Optuna P2] Searching parameters ({n_optuna_trials_p2} trials/model)...')
        tune_skf = StratifiedKFold(n_splits=3, shuffle=True, random_state=42)

        def objective_lgb_p2(trial, X=X_train_p2, yy=y):
            params = {
                'objective':'binary','metric':'binary_logloss','boosting_type':'gbdt',
                'n_estimators':300,'verbose':-1,'n_jobs':-1,'random_state':42,
                'learning_rate':    trial.suggest_float('learning_rate', 0.01, 0.1, log=True),
        ...
        def objective_xgb_p2(trial, X=X_train_p2, yy=y):
            params = {
                'objective':'binary:logistic','eval_metric':'logloss',
                'n_estimators':300,'n_jobs':-1,'random_state':42,
                'learning_rate':   trial.suggest_float('learning_rate', 0.01, 0.1, log=True),
        ...
        def objective_cat_p2(trial, X=X_train_p2, yy=y):
            params = {
                'loss_function':'Logloss','iterations':300,'verbose':False,
                'thread_count':-1,'random_seed':42,
                'learning_rate':trial.suggest_float('learning_rate', 0.01, 0.1, log=True),
        ...
        study_lgb = optuna.create_study(direction='minimize'); study_lgb.optimize(objective_lgb_p2, n_trials=n_optuna_trials_p2)
        study_xgb = optuna.create_study(direction='minimize'); study_xgb.optimize(objective_xgb_p2, n_trials=n_optuna_trials_p2)
        study_cat = optuna.create_study(direction='minimize'); study_cat.optimize(objective_cat_p2, n_trials=n_optuna_trials_p2)
        print(f'    LGBM best: {study_lgb.best_value:.4f}  XGB: {study_xgb.best_value:.4f}  CAT: {study_cat.best_value:.4f}')

        best_lgb, best_xgb, best_cat = _build_best_params(study_lgb, study_xgb, study_cat)

        print(f'  [P2] Training {len(seeds)}-seed ensemble...')
        target_oof, target_test = _run_l0_l1(
            X_train_p2, X_test_p2, train_full, test_full, y, target,
            best_lgb, best_xgb, best_cat, seeds, n_folds,
            meta_params, te_windows, USE_FOLD_SAFE_TE, 'P2')
