# 정세진의 base_v4 기반 수정
# 변경사항:
#   v18: v17(GBM+LSTM)에서 LSTM → LSTM+Temporal Attention으로 교체
#        Attention: 7일 각 타임스텝의 중요도를 학습하여 weighted sum
#        최종 예측 = GBM 85% + LSTM-Attention 15%
...
class DailyLSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        weights = torch.softmax(scores, dim=1)         # 날짜별 중요도
        context = (out * weights).sum(dim=1)           # (batch, hidden)
        return torch.sigmoid(self.fc(self.drop(context))).squeeze(-1)
...
USE_TRAIN_SUBJ_NORM = os.environ.get('V18_TRAIN_NORM', '0') == '1'
USE_RANK_BLEND      = os.environ.get('V18_RANK_BLEND', '0') == '1'
USE_FOLD_SAFE_TE    = os.environ.get('V18_FOLD_SAFE_TE', '1') == '1'
USE_CALIBRATION     = os.environ.get('V18_CALIBRATION', '0') == '1'
CALIBRATION_METHOD  = os.environ.get('V18_CALIB_METHOD', 'platt').strip().lower()
if CALIBRATION_METHOD not in {'platt', 'isotonic'}:
    raise ValueError("V18_CALIB_METHOD must be one of: platt, isotonic")
PSEUDO_PUBLIC_TAIL_FRAC = float(os.environ.get('V18_PSEUDO_TAIL_FRAC', '0.2'))
if not (0 < PSEUDO_PUBLIC_TAIL_FRAC < 1):
    raise ValueError("V18_PSEUDO_TAIL_FRAC must be in (0, 1)")

FORCE_CPU = os.environ.get('V18_FORCE_CPU', '0') == '1'
...
OUTPUT_PATH     = OUTPUT_DIR  / 'submission_v18.csv'
REPORT_PATH     = REPORT_DIR  / 'report_v18.txt'
SUMMARY_PATH    = SUMMARY_DIR / 'summary_v18.json'
OOF_PATH        = OOF_DIR     / 'oof_v18.csv'
TEST_PREDS_PATH = REPORT_DIR  / 'test_preds_v18.csv'
RUN_LOG_PATH    = LOG_DIR     / 'run_v18.log'
...
def main():
    ...
    print('Starting v18 Training Pipeline...')
    ...
    print(f'\n{"=" * 55}')
    print(f'v18 Total OOF:         {oof_total:.4f}')
    print(f'v18 Pseudo-public OOF: {pseudo_oof_total:.4f}')
    print(f'{"=" * 55}')
    ...
    summary = {
        'exp_tag': 'v18_gbm_lstm_attention',
