"""
제 5회 ETRI 휴먼이해 인공지능 논문경진대회 - Baseline v7
...
USE_APP_CATEGORY      = os.environ.get('USE_APP_CATEGORY',      '1') == '1'
USE_PERSONAL_RELATIVE = os.environ.get('USE_PERSONAL_RELATIVE', '1') == '1'
USE_HR_FREQ           = os.environ.get('USE_HR_FREQ',           '1') == '1'
...
OUTPUT_PATH = str(OUTPUTS_DIR / 'submissions' / f'submission_v8{EXP_TAG}.csv')
REPORT_PATH = str(OUTPUTS_DIR / 'report' / f'report_v8{EXP_TAG}.txt')


TARGET_COLS     = ['Q1', 'Q2', 'Q3', 'S1', 'S2', 'S3', 'S4']
N_SPLITS        = 10
OPTUNA_TRIALS   = 100
SEED_LIST = [42, 123, 777, 2024, 31337, 9999, 7, 314]
SEED            = 42
...
def add_personal_relative_features(train_df, test_df, feat_cols):
    print("  [v9] personal_relative (Transductive Z-score) ...")
    ...
    # [수정됨] Train과 Test를 합쳐서 평균/표준편차 계산
    combined = pd.concat([train_df, test_df], ignore_index=True)
    subj_mean = combined.groupby('subject_id')[candidate].mean()
    subj_std  = combined.groupby('subject_id')[candidate].std()
...
def add_target_encoding(train_df, test_df):
    print("  [v9] Target Encoding (합법적 Target Lag) ...")
    train_tmp = train_df.copy()
    test_tmp  = test_df.copy()

    # Test의 정답은 모름! 반드시 NaN으로 덮어씌워 Leakage 완벽 차단
    for t in TARGET_COLS:
        if t in test_tmp.columns:
            test_tmp[t] = np.nan

    train_tmp['_is_test'] = 0
    test_tmp['_is_test']  = 1
    combined = pd.concat([train_tmp, test_tmp], ignore_index=True)
    # 시계열 정석: 날짜순 정렬
    combined = combined.sort_values(['subject_id', 'lifelog_date']).reset_index(drop=True)

    for t in TARGET_COLS:
        # 1. 어제의 정답 (Lag 1)
        combined[f'{t}_lag1'] = combined.groupby('subject_id')[t].shift(1)
        # 2. 최근 3일, 7일, 14일 이동평균 (과거의 흐름 파악)
        for w in [3, 7, 14]:
            combined[f'{t}_enc_roll{w}'] = combined.groupby('subject_id')[t].transform(
                lambda x: x.shift(1).rolling(w, min_periods=1).mean()
            )

    train_out = combined[combined['_is_test'] == 0].drop('_is_test', axis=1)
    test_out  = combined[combined['_is_test'] == 1].drop('_is_test', axis=1)
...
def add_lag_features(train_df, test_df):
    safe_lag_cols = [
        'sleep_duration_min', 'usage_total_time', 'night_hr_mean',
        'wPedo_step_mean', 'light_night_mean', 'hr_rmssd'
    ]

    # train + test 합쳐서 날짜 순으로 shift
    ...
    for c in safe_lag_cols:
        if c not in combined.columns:
            print(f"    [lag skip] {c} 컬럼 없음")
            continue
        grp = combined.groupby('subject_id')[c]
        combined[f'{c}_lag1'] = grp.shift(1)
        combined[f'{c}_lag2'] = grp.shift(2)
...
def add_subject_stats(train_df, test_df, feat_cols):
    ...
    # [수정됨] Train과 Test를 합쳐서(Transductive) 피험자 통계 계산
    combined = pd.concat([train_df, test_df], ignore_index=True)
    stats = combined.groupby('subject_id')[num_cols].agg(['mean','std']).reset_index()
...
def train_predict_ensemble_with_pl(X, y, X_test, name, lgb_p, xgb_p, cat_p):
    """1차 학습으로 퀄리티 높은 Test 샘플을 추출해 2차 학습에 추가하는 PL 함수"""
    print("      [1차 학습] Pseudo Label 추출용 단일 모델 학습 중...")
    test_p1, oof_p1, _ = train_predict_seed(X, y, X_test, name, lgb_p, xgb_p, cat_p, SEED_LIST[0])

    PL_THRES_HIGH = 0.85 
    PL_THRES_LOW  = 0.15 

    pseudo_idx = np.where((test_p1 >= PL_THRES_HIGH) | (test_p1 <= PL_THRES_LOW))[0]
    if len(pseudo_idx) > 0:
        print(f"      [Pseudo Labeling] {len(pseudo_idx)}개의 확실한 Test 샘플 훈련 데이터에 추가!")
        X_pseudo = X_test.iloc[pseudo_idx].copy()
        y_pseudo = np.where(test_p1[pseudo_idx] >= PL_THRES_HIGH, 1, 0)
        X_extended = pd.concat([X, X_pseudo], axis=0).reset_index(drop=True)
        y_extended = pd.Series(np.concatenate([y.values, y_pseudo]))
    else:
        print("      [Pseudo Labeling] 기준을 넘는 확신 샘플이 없어 원본으로 진행합니다.")
        X_extended, y_extended = X, y

    print("      [2차 학습] 확장된 데이터로 최종 앙상블 5-Seed 학습 중...")
    test_preds, oof_preds, seed_losses = [], [], []
    for seed in SEED_LIST:
        test_p, oof_p_ext, seed_l = train_predict_seed(X_extended, y_extended, X_test, name, lgb_p, xgb_p, cat_p, seed)
        test_preds.append(test_p)
        oof_preds.append(oof_p_ext[:len(X)]) 
        loss_val = log_loss(y, oof_p_ext[:len(X)])
        seed_losses.append(loss_val)
        print(f"        seed={seed:5d} | {name} | OOF: {loss_val:.4f}")

    final_test = np.mean(test_preds, axis=0)
    final_oof  = np.mean(oof_preds, axis=0)
    final_loss = log_loss(y, np.clip(final_oof, 1e-7, 1-1e-7))
    print(f"  >> PL Seed-Ensemble OOF: {final_loss:.4f} (std={np.std(seed_losses):.4f})")
    return final_test, final_oof, final_loss, seed_losses
...
def write_report(report_data, path=REPORT_PATH):
    lines = []
    lines.append("="*80)
    lines.append(f"제 5회 ETRI 휴먼이해 AI 논문경진대회 - 실험 리포트 (v8{EXP_TAG})")
    ...
    for t in TARGET_COLS:
        d = report_data['per_target'][t]
        lines.append(f"\n[{t}]  LGB Fixed params: {json.dumps(d['lgb_params'])}")
...
def main():
    ...
    train_df, test_df = add_target_encoding(train_df, test_df)
    ...
    for t in TARGET_COLS:
        ...
        print(f"    [LGB Only Seed Ensemble] {len(SEED_LIST)} seeds × {N_SPLITS}-fold CV")
        test_preds_all, oof_preds_all, seed_losses = [], [], []

        for seed in SEED_LIST:
            skf = StratifiedKFold(n_splits=N_SPLITS, shuffle=True, random_state=seed)
            test_p_seed = np.zeros(len(X_test))
            oof_p_seed  = np.zeros(len(X))

            for tr_i, val_i in skf.split(X, y):
                X_tr, y_tr   = X.iloc[tr_i], y.iloc[tr_i]
                X_val, y_val = X.iloc[val_i], y.iloc[val_i]

                model = LGBMClassifier(**LGB_FIXED, random_state=seed)
                model.fit(X_tr, y_tr, eval_set=[(X_val, y_val)],
                          callbacks=[early_stopping(50), log_evaluation(-1)])

                lgb_t, lgb_v = calibrate(
                    model.predict_proba(X_val)[:,1], y_val,
                    model.predict_proba(X_test)[:,1])

                test_p_seed      += lgb_t / N_SPLITS
                oof_p_seed[val_i] = lgb_v

            seed_loss = log_loss(y, np.clip(oof_p_seed, 1e-7, 1-1e-7))
            seed_losses.append(seed_loss)
            test_preds_all.append(test_p_seed)
            oof_preds_all.append(oof_p_seed)
            print(f"    seed={seed:5d} | {t} | OOF: {seed_loss:.4f}")

        final_test = np.mean(test_preds_all, axis=0)
        final_oof  = np.mean(oof_preds_all,  axis=0)
        final_loss = log_loss(y, np.clip(final_oof, 1e-7, 1-1e-7))
        print(f"  >> Ensemble OOF: {final_loss:.4f}")

        sub[t] = np.clip(final_test, 0.02, 0.98)
        losses.append(final_loss)

        report_data['per_target'][t] = {
            'oof':         final_loss,
            'seed_losses': seed_losses,
            'lgb_params':  LGB_FIXED,
    ...
    with open(f'./summary_v8{EXP_TAG}.json', 'w') as f:
