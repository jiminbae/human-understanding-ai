# 정세진의 base_v4 기반 수정
# 변경사항:
#   v16: base_v4 파라미터 유지 + 미활용 센서 피처 추가
#        - extract_hr: 수면시간대(0-6h) HRV 피처 (rmssd, sdnn, pnn50)
#        - extract_ambience: Vehicle/Inside_large_room/Speech/Silence frac 추가
#        - extract_usage: 앱 카테고리별 사용시간 (game, sns) 추가
...
USE_TRAIN_SUBJ_NORM = os.environ.get('V16_TRAIN_NORM', '0') == '1'
USE_RANK_BLEND      = os.environ.get('V16_RANK_BLEND', '0') == '1'
USE_FOLD_SAFE_TE    = os.environ.get('V16_FOLD_SAFE_TE', '1') == '1'
USE_CALIBRATION     = os.environ.get('V16_CALIBRATION', '0') == '1'
CALIBRATION_METHOD  = os.environ.get('V16_CALIB_METHOD', 'platt').strip().lower()
if CALIBRATION_METHOD not in {'platt', 'isotonic'}:
    raise ValueError("V16_CALIB_METHOD must be one of: platt, isotonic")
PSEUDO_PUBLIC_TAIL_FRAC = float(os.environ.get('V16_PSEUDO_TAIL_FRAC', '0.2'))
if not (0 < PSEUDO_PUBLIC_TAIL_FRAC < 1):
    raise ValueError("V16_PSEUDO_TAIL_FRAC must be in (0, 1)")

FORCE_CPU = os.environ.get('V16_FORCE_CPU', '0') == '1'
...
OUTPUT_PATH     = OUTPUT_DIR  / 'submission_v16.csv'
REPORT_PATH     = REPORT_DIR  / 'report_v16.txt'
SUMMARY_PATH    = SUMMARY_DIR / 'summary_v16.json'
OOF_PATH        = OOF_DIR     / 'oof_v16.csv'
TEST_PREDS_PATH = REPORT_DIR  / 'test_preds_v16.csv'
RUN_LOG_PATH    = LOG_DIR     / 'run_v16.log'
...
def extract_hr(df_raw, keys):
    ...
    for (sid, d), grp in df_raw.groupby(['subject_id', 'date']):
        row = {'subject_id': sid, 'lifelog_date': d}
        sleep_grp = grp[grp['timestamp'].dt.hour < 6]
        if len(sleep_grp) >= 10:
            hr_vals = np.concatenate([np.asarray(v, dtype=float) for v in sleep_grp['heart_rate']])
            hr_vals = hr_vals[(hr_vals > 30) & (hr_vals < 220)]
            if len(hr_vals) >= 60:
                rr = 60000.0 / hr_vals
                rr_diff = np.diff(rr)
                row['hrv_rmssd_sleep'] = float(np.sqrt(np.mean(rr_diff ** 2)))
                row['hrv_sdnn_sleep']  = float(np.std(rr))
                row['hrv_pnn50_sleep'] = float(np.mean(np.abs(rr_diff) > 50) * 100)
        feats.append(row)
    return pd.DataFrame(feats)
...
_SNS_APPS  = {'카카오톡', 'KakaoTalk', 'Instagram', 'YouTube', '유튜브', 'NAVER',
              '네이버', 'Facebook', 'Twitter', 'TikTok', '틱톡', '카카오스토리', '밴드'}
_GAME_APPS = {'게임', 'Game', 'PUBG', '배틀그라운드', '리그오브레전드', '포켓몬GO'}

def _categorize_app(name):
    n = str(name)
    for s in _SNS_APPS:
        if s in n: return 'sns'
    for g in _GAME_APPS:
        if g in n: return 'game'
    return 'other'
...
def extract_usage(df_raw, keys):
    ...
    for (sid, d), grp in df_raw.groupby(['subject_id', 'date']):
        row = {'subject_id': sid, 'lifelog_date': d}
        total_time = late_time = eve_time = n_apps = sns_time = game_time = 0
        for ts, v in zip(grp['timestamp'], grp['m_usage_stats']):
            items = list(v) if hasattr(v, '__iter__') and not isinstance(v, dict) else []
            for app in items:
                if not isinstance(app, dict): continue
                t = int(app.get('total_time', 0) or 0)
                total_time += t; n_apps += 1
                cat = _categorize_app(app.get('app_name', ''))
                if cat == 'sns':  sns_time  += t
                if cat == 'game': game_time += t
        ...
        row['usage_sns_min']    = sns_time  / 60
        row['usage_game_min']   = game_time / 60
        row['usage_sns_ratio']  = sns_time  / (total_time + 1)
        feats.append(row)
    return pd.DataFrame(feats)
...
_AMB_LABELS = ['Music', 'Speech', 'Silence', 'Vehicle', 'Inside, large room or hall']
...
def extract_ambience(df_raw, keys):
    ...
    for (sid, d), grp in df_raw.groupby(['subject_id', 'date']):
        row = {'subject_id': sid, 'lifelog_date': d}
        scores  = {l: [] for l in _AMB_LABELS}
        present = {l: 0  for l in _AMB_LABELS}
        n = 0
        for v in grp['m_ambience']:
            ...
            for item in items:
                try:
                    label_map[str(item[0])] = float(item[1])
                except: pass
            for l in _AMB_LABELS:
                if l in label_map:
                    scores[l].append(label_map[l])
                    present[l] += 1
            n += 1
        for l in _AMB_LABELS:
            key = l.replace(', ', '_').replace(' ', '_').lower()
            row[f'amb_{key}_mean'] = np.mean(scores[l]) if scores[l] else np.nan
            row[f'amb_{key}_frac'] = present[l] / n     if n > 0    else np.nan
        row['amb_n_records'] = n
        feats.append(row)
    return pd.DataFrame(feats)
...
def write_report(report_data):
    lines = [
        '=' * 80,
        'Baseline v16 run report',
        f"Generated: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        'Base: base_v4 파라미터 + HRV sleep / Ambience / App category 피처',
...
def main():
    ...
    print('Starting v16 Training Pipeline...')
    ...
    print(f'\n{"=" * 55}')
    print(f'v16 Total OOF:         {oof_total:.4f}')
    print(f'v16 Pseudo-public OOF: {pseudo_oof_total:.4f}')
    print(f'{"=" * 55}')
    ...
    summary = {
        'exp_tag': 'v16_hrv_ambience_appcat',
