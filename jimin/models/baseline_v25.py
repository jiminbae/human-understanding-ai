# v25: v23 + 요일별 fold-safe target encoding (dow TE)
#   - 같은 요일의 과거 N주 타겟 평균 → 요일 주기 패턴 포착
#   - dow_windows=[2,4]: 최근 2주/4주 같은 요일 평균
#   - 기존 lag TE(3/7/14/21일)와 별도로 fold 내부에서 concat
#   - stability filter는 기존 429 base features에만 적용 (TE는 fold 내 동적 추가)
import warnings
...
def build_fold_safe_dow_encoding(train_hist_df, tr_query_df, val_query_df, test_query_df, target, windows):
    """같은 요일의 과거 N주 타겟 평균 (fold-safe)."""
    h = train_hist_df[['subject_id','lifelog_date',target]].sort_values(
        ['subject_id','lifelog_date']).reset_index(drop=True)
    hist = {}
    for sid, grp in h.groupby('subject_id'):
        hist[sid] = {
            'dates':  grp['lifelog_date'].to_numpy(),
            'dows':   grp['lifelog_date'].dt.dayofweek.to_numpy(),
            'labels': grp[target].to_numpy(),
        }

    def _encode(query_df):
        rows = []
        for sid, d in query_df[['subject_id','lifelog_date']].itertuples(index=False):
            row = {}
            if sid not in hist:
                for w in windows: row[f'dow_te{w}'] = np.nan
                rows.append(row); continue
            info   = hist[sid]
            k      = np.searchsorted(info['dates'], d, side='left')
            mask   = info['dows'][:k] == d.dayofweek
            same   = info['labels'][:k][mask]
            for w in windows:
                row[f'dow_te{w}'] = np.nanmean(same[-w:]) if len(same) > 0 else np.nan
            rows.append(row)
        return pd.DataFrame(rows, index=query_df.index)

    return _encode(tr_query_df), _encode(val_query_df), _encode(test_query_df)
...
def train_and_predict(train_full, test_full, feature_cols):
    ...
    dow_windows     = [2, 4]  # 같은 요일 최근 2주 / 4주 평균
        ...
        for seed in seeds:
            ...
            for tr_idx, val_idx in skf.split(X_train_use, y):
                ...
                if USE_FOLD_SAFE_TE:
                    ...
                    tr_dow, val_dow, test_dow = build_fold_safe_dow_encoding(
                        hist_df, tr_query, val_query, test_query, target, dow_windows)
                    X_tr  = pd.concat([X_tr.reset_index(drop=True),  tr_te.reset_index(drop=True),  tr_dow.reset_index(drop=True)],  axis=1)
                    X_val = pd.concat([X_val.reset_index(drop=True), val_te.reset_index(drop=True), val_dow.reset_index(drop=True)],  axis=1)
                    X_te  = pd.concat([X_te.reset_index(drop=True),  test_te.reset_index(drop=True), test_dow.reset_index(drop=True)], axis=1)
