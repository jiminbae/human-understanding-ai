...
# v12: reliable validation + probability quality focused setup.
...
if USE_FOLD_SAFE_TE:
	_tag_parts.append('foldsafe_te')
if USE_CALIBRATION:
	_tag_parts.append(f'calib_{CALIBRATION_METHOD}')
EXP_TAG = '_public_v12_' + ('_'.join(_tag_parts) if _tag_parts else 'base')
...
def build_feature_table(train_df, sub_df):
	...
	# v12: target encoding is generated fold-by-fold in train_and_predict to avoid leakage.
	...
	for ti, target in enumerate(TARGETS):
		...
		for seed in seeds:
			...
			for tr_idx, val_idx in skf.split(X_train_base, y):
				...
				seed_test_prob += model.predict_proba(X_te)[:, 1] / n_folds
...
def write_report(report_data):
	lines = []
	lines.append('=' * 80)
	lines.append('Baseline v12 run report')
	lines.append(f"Generated: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
	lines.append('Base: public-stable reference + validation reliability focus')
	lines.append(f'Toggle V12_TRAIN_NORM: {USE_TRAIN_SUBJ_NORM}')
	lines.append(f'Toggle V12_RANK_BLEND: {USE_RANK_BLEND}')
	lines.append(f'Toggle V12_FOLD_SAFE_TE: {USE_FOLD_SAFE_TE}')
	lines.append(f'Toggle V12_CALIBRATION: {USE_CALIBRATION} ({CALIBRATION_METHOD})')
	lines.append(f'Pseudo tail frac: {PSEUDO_PUBLIC_TAIL_FRAC}')
...
def main():
	...
	try:
		print('Base: public-stable reference + validation reliability focus')
		print(f'V12_TRAIN_NORM={int(USE_TRAIN_SUBJ_NORM)}')
		print(f'V12_RANK_BLEND={int(USE_RANK_BLEND)}')
		print(f'V12_FOLD_SAFE_TE={int(USE_FOLD_SAFE_TE)}')
		print(f'V12_CALIBRATION={int(USE_CALIBRATION)} ({CALIBRATION_METHOD})')
		print(f'V12_PSEUDO_TAIL_FRAC={PSEUDO_PUBLIC_TAIL_FRAC}')
		print(f'V12_FORCE_CPU={int(FORCE_CPU)}')
		...
		print(f'\n{"=" * 55}')
		print(f'v12 Total OOF: {oof_total:.4f}')
		print(f'v12 Pseudo-public OOF: {pseudo_oof_total:.4f}')
		print(f'{"=" * 55}')
		for t in TARGETS:
		...
		summary = {
			'base': 'public-stable-reference-v12',
