# 정세진의 base_v4 기반 수정
# 변경사항:
#   v15: base_v4(foldsafe_te only) 파라미터 유지 + TabPFN을 4번째 모델로 추가
#        LGB 35% + XGB 25% + CAT 25% + TabPFN 15%
...
USE_TRAIN_SUBJ_NORM = os.environ.get('V15_TRAIN_NORM', '0') == '1'
USE_RANK_BLEND      = os.environ.get('V15_RANK_BLEND', '0') == '1'
USE_FOLD_SAFE_TE    = os.environ.get('V15_FOLD_SAFE_TE', '1') == '1'
USE_CALIBRATION     = os.environ.get('V15_CALIBRATION', '0') == '1'
CALIBRATION_METHOD  = os.environ.get('V15_CALIB_METHOD', 'platt').strip().lower()
if CALIBRATION_METHOD not in {'platt', 'isotonic'}:
    raise ValueError("V15_CALIB_METHOD must be one of: platt, isotonic")
PSEUDO_PUBLIC_TAIL_FRAC = float(os.environ.get('V15_PSEUDO_TAIL_FRAC', '0.2'))
if not (0 < PSEUDO_PUBLIC_TAIL_FRAC < 1):
    raise ValueError("V15_PSEUDO_TAIL_FRAC must be in (0, 1)")

FORCE_CPU = os.environ.get('V15_FORCE_CPU', '0') == '1'
...
OUTPUT_PATH     = OUTPUT_DIR  / 'submission_v15.csv'
REPORT_PATH     = REPORT_DIR  / 'report_v15.txt'
SUMMARY_PATH    = SUMMARY_DIR / 'summary_v15.json'
OOF_PATH        = OOF_DIR     / 'oof_v15.csv'
TEST_PREDS_PATH = REPORT_DIR  / 'test_preds_v15.csv'
RUN_LOG_PATH    = LOG_DIR     / 'run_v15.log'
...
def train_and_predict(train_full, test_full, feature_cols):
    ...
    w_lgb, w_xgb, w_cat, w_tabpfn = 0.35, 0.25, 0.25, 0.15
...
def write_report(report_data):
    lines = [
        '=' * 80,
        'Baseline v15 run report',
        f"Generated: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        'Base: base_v4 파라미터 + TabPFN 4번째 모델 (LGB35%+XGB25%+CAT25%+TabPFN15%)',
...
def main():
    ...
    print('Starting v15 Training Pipeline...')
    ...
    print(f'\n{"=" * 55}')
    print(f'v15 Total OOF:         {oof_total:.4f}')
    print(f'v15 Pseudo-public OOF: {pseudo_oof_total:.4f}')
    print(f'{"=" * 55}')
    ...
    summary = {
        'exp_tag': 'v15_quad_ensemble_tabpfn',
